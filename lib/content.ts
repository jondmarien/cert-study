...
import { lessonFrontmatterSchema } from "@/lib/schema";
import { stripMarkup } from "@/lib/text";
import { familyNames, getTrack, TRACK_LIST } from "@/lib/tracks";
import type {
  Heading,
  Lesson,
  LessonMeta,
...
export function getTrackLessons(track: TrackId): Lesson[] {
  return getAllLessons().filter((lesson) => lesson.track === track);
}
...
export function resolveRelated(lesson: LessonMeta): ResolvedRelated[] {
  ...
  return lesson.related.map((link) => {
    const target = all.find(
      (item) => item.track === link.track && item.slug === link.slug,
    );
    if (!target) {
