...
import { tallyProgress, type ProgressState } from "@/lib/progress";
...
export default function HomeProgress({
  ...
  const [progress, setProgress] = useState<ProgressState>({ statuses: {}, last: null });
      ...
      {last ? (
        <section className="rounded-2xl border border-accent bg-soft px-5 py-4">
          ...
          <h2 className="mt-1 font-display text-3xl">
            <a href={last.href} className="underline decoration-accent/40 underline-offset-4">
              {last.title}
            </a>
          </h2>
        ...
        {cards.map((card) => {
          ...
          return (
            <article
              ...
              style={{
                borderTopWidth: 4,
                borderTopColor: card.id === "bscp" ? "var(--accent)" : "var(--copper)",
              }}
            >
              ...
              <h2 className="mt-2 font-display text-4xl leading-none">
                <a href={card.href} className="hover:text-accent">
                  {card.shortTitle}
                </a>
              </h2>
